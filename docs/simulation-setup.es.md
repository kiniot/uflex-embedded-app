...
### 1. Definir el Mapa del Firmware Target (`wokwi.toml`)
Asegúrate de tener el siguiente archivo en la raíz del proyecto llamado `wokwi.toml` con las siguientes definiciones estructurales para enrutar el simulador hacia tus binarios compilados por PlatformIO:
...
> **Importante:** Asegúrate de hacer **Build** del proyecto con PlatformIO al menos una vez antes de lanzar la simulación. Si no compilas el código primero, las carpetas internas y los archivos `firmware.bin` y `firmware.elf` no existirán en el disco, por lo que el simulador no podrá reconocer las rutas y lanzará un error de inicialización.

### 2. Definir el Esquema del Circuito Electrónico (`diagram.json`)
...
1. Asegúrate de que tu código fuente compile limpiamente haciendo clic en el ícono de verificación de PlatformIO Build (`✓`) en la barra de herramientas inferior.
...
1. **Paso Obligatorio de Compilación:** Dirígete primero a la pestaña **PlatformIO** y realiza un **Build** del proyecto para generar el firmware actualizado en el almacenamiento local.
